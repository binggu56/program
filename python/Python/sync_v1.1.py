...
# Local
script_dir = '/Users/bing/Documents/Python' # Dir script is located in
...
def backup(dir, file, remote_host, remote_dir, remote_user, script_dir):
    ...
    os.chdir(dir)
    cmd = "tar -czf %s/%s --exclude-from=%s/backup_exclude.txt ."% (script_dir, file, script_dir)
    os.system(cmd)
    ...
    if do_scp:
        cmd = "scp -Bq %s %s@%s:%s/%s" % (file, remote_user, remote_host, remote_dir, file)
    #print cmd
    os.system(cmd)
...
if backup_home:
    ...
    put, get = subprocess.Popen(cmd) # Get a listing of directories and sizes
